...
AGBPlayerCameraManager::AGBPlayerCameraManager(const class FObjectInitializer& ObjectInitializer)
...
void AGBPlayerCameraManager::BeginPlay()
{
	Super::BeginPlay();
...
void AGBPlayerCameraManager::UpdateCamera(float DeltaTime)
{
	AGreyBlockHeroCharacter* MyPawn = PCOwner ? Cast<AGreyBlockHeroCharacter>(PCOwner->GetPawn()) : nullptr;
	if (MyPawn)
	{
